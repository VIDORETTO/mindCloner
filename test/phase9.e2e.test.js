...
test("Fase 9 E2E: executa psicologia profunda e transiciona para fase 10", async () => {
  const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mindclone-test-"));
  const profileId = "clara-mendes-v9k4";
  ...
  await runSession({ profileId, baseDir: tmpRoot, io: phase1Io });
  ...
  await runSession({ profileId, baseDir: tmpRoot, io: phase2Io });
  ...
  await runSession({ profileId, baseDir: tmpRoot, io: phase3Io });
  ...
  await runSession({ profileId, baseDir: tmpRoot, io: phase4Io });
  ...
  await runSession({ profileId, baseDir: tmpRoot, io: phase5Io });
  ...
  await runSession({ profileId, baseDir: tmpRoot, io: phase6Io });
  ...
  await runSession({ profileId, baseDir: tmpRoot, io: phase7Io });
  ...
  const phase9Io = createScriptedIO([
    "Autonomia, contribuicao, crescimento",
    "Curiosidade, aprender, servir pessoas",
    "Responsabilidade com minha familia",
    "Tomar decisao errada que afete quem depende de mim",
    "Perder relevancia, falhar com minha familia",
    "Construir algo com impacto, viver com mais liberdade",
    "Liderar uma empresa, escrever um livro",
    "sim",
    "Usar minha capacidade para gerar impacto humano e tecnico.",
    "Diria que sou intensa, leal e estrategica.",
    "7",
    "6",
    "Mais leve, corajosa e consistente.",
    "Medo de decepcionar vs vontade de ousar.",
    "Nao ser suficiente, perder controle",
    "8",
    "Voce deveria ter previsto isso, Nao erre de novo",
    "Tenho mais respeito e menos julgamento por quem eu era.",
    "Mudanca de cidade, virada para lideranca, nascimento dos filhos",
    "Mentoria forte, fracasso de projeto, reconstrucao emocional",
    "Promocao a lider, apoiar minha familia, superar burnout",
    "Ter adiado conversas importantes, escolher seguranca por medo",
    "Perdao em relacoes antigas, equilibrar ambicao e descanso",
    "Como alguem que uniu competencia com humanidade",
  ]);
  const phase9Result = await runSession({ profileId, baseDir: tmpRoot, io: phase9Io });

  assert.ok(
    phase9Io.outputs.some((line) =>
      line.includes("sinta-se livre para responder como quiser ou pular")
    ),
    "A Fase 9 deve normalizar perguntas profundas com opcao de pular"
  );
  assert.ok(
    phase9Io.outputs.some((line) => line.includes("Obrigado por compartilhar isso, nao e facil.")),
    "A Fase 9 deve validar emocao apos respostas sensiveis"
  );
  assert.equal(phase9Result.phaseTransitioned, true);
  assert.equal(phase9Result.state.current_phase, 10);
  assert.equal(phase9Result.state.phases_status.phase_09.status, "completed");
  assert.match(phase9Result.summary, /Motivacao central:/);
  ...
  assert.equal(profileRaw.motivations_and_drives.purpose_sense.has_clear_purpose, true);
  assert.equal(profileRaw.self_concept.inner_critic.intensity, 8);
  assert.equal(
    profileRaw.life_narrative.legacy_desire,
    "Como alguem que uniu competencia com humanidade"
  );
  assert.equal(profileRaw.life_narrative.turning_points[0].event, "Mudanca de cidade");
