...
test("Fase 7 E2E: executa valores e crencas e transiciona para fase 8", async () => {
  const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mindclone-test-"));
  const profileId = "clara-mendes-v7k9";
  ...
  assert.ok(
    phase7Io.outputs.some((line) => line.includes("aos seus filhos")),
    "A Fase 7 deve personalizar pergunta de valores com contexto familiar da Fase 1"
  );
  assert.ok(
    phase7Io.outputs.some((line) => line.includes("seus valores profissionais")),
    "A Fase 7 deve cruzar contexto de valores profissionais das fases anteriores"
  );
  assert.equal(phase7Result.phaseTransitioned, true);
  assert.equal(phase7Result.state.current_phase, 8);
  assert.equal(phase7Result.state.phases_status.phase_07.status, "completed");
  assert.match(phase7Result.summary, /Valores core:/);
  ...
  assert.equal(
    profileRaw.values_and_beliefs.worldview.summary,
    "Acredito em progresso com empatia e responsabilidade coletiva."
  );
  assert.equal(profileRaw.values_and_beliefs.moral_foundations.fairness_cheating, 9);
  assert.equal(profileRaw.values_and_beliefs.spiritual_religious.importance, 6);
  assert.deepEqual(profileRaw.values_and_beliefs.philosophical_stances, [
    "Estoicismo",
    "humanismo secular",
  ]);
  assert.equal(profileRaw.values_and_beliefs.core_values[0].value, "Integridade");
  assert.equal(profileRaw.values_and_beliefs.core_values[0].importance, 10);
  assert.equal(profileRaw.meta.last_phase, "phase_08");
});
