...
Status geral: Execucao avancada (P0-P3 concluidos, P4 planejado para UX final da CLI)
...
## P4 - CLI de produto final para usuario comum (novo escopo)

- [ ] Navegacao interativa por setas com menu principal e ajuda contextual.
- [ ] Wizard de primeiro uso para configuracao inicial sem flags manuais.
- [ ] Entrevista com dois modos: faseado (10 fases) e conversacional adaptativo.
- [ ] Comandos em sessao: `/help`, `/status`, `/save`, `/new`, `/pause`, `/menu`.
- [ ] Fluxo anti-alucinacao com handoff de agente via `/save` e retomada via `/new`.
- [ ] Centro de exportacao no menu para gerar documento completo e resumo para IA.
- [ ] Configuracao IA guiada com provider OpenAI e modelo fixo `gpt-5-mini-2025-08-07`.

#### Plano de implementacao P4.1 (planejamento atual)

- [x] Definir escopo detalhado, arquitetura, contratos de dados, fases e criterios de aceite.
- [x] Publicar documento tecnico dedicado em `tasks/plano-cli-entrevista-dinamica.md`.
- [ ] Validar plano com usuario antes de iniciar codificacao.
- [ ] Executar F0/F1 com primeira demo navegavel da nova CLI.
...
- [ ] Ciclo 8: P4.1 CLI dinamica para usuario final (menus por setas + handoff `/save`/`/new` + config guiada).
...
## 6.6) Plano de execucao do ciclo atual (CLI dinamica anti-burro + handoff de agente)

- [x] Mapear estado atual do projeto para reaproveitar modulos e evitar regressao.
- [x] Especificar plano completo em `tasks/plano-cli-entrevista-dinamica.md` com arquitetura, fluxos e backlog.
- [ ] Validar plano com o usuario antes de iniciar implementacao.
- [ ] Iniciar F0/F1: shell TUI com setas e menu principal com ajuda contextual.
- [ ] Iniciar F2: setup guiado com OpenAI (`gpt-5-mini-2025-08-07`) e persistencia de configuracao.

## 7) Revisao da entrega (preencher a cada ciclo)

### Ciclo atual

- Escopo executado: planejamento completo do novo escopo P4 para transformar a CLI em experiencia de produto final com menus por setas, setup guiado, entrevista adaptativa e fluxo `/save` + `/new`.
- Resultado objetivo: backlog P4 pronto para execucao incremental, com criterios de aceite claros e ordem de entrega sem ambiguidades.
- Evidencias (testes/comandos): publicacao de `tasks/plano-cli-entrevista-dinamica.md`; atualizacao de `tasks/todo.md` com checklist P4.1.
- Riscos remanescentes: implementacao TUI pode impactar compatibilidade non-TTY se nao houver adapter de IO adequado.
- Proxima acao recomendada: validar este plano com o usuario e iniciar F0/F1 imediatamente.
